...
class TurtleControllerNode(Node):
    ...
    def callback_pose(self, pose: Pose):
        if self.is_active_:
            ...
            if pose.x > 5.544445 and self.previous_x_ <= 5.544445:
                self.previous_x_ = pose.x
                self.get_logger().info("Set color to color_1.")
                self.call_set_pen(self.color_1_[0],self.color_1_[1],self.color_1_[2])
            elif pose.x <= 5.544445 and self.previous_x_ > 5.544445:
                self.previous_x_ = pose.x
                self.get_logger().info("Set color to color_2.")
